...
<!-- 	<col width="200">
	...
	<tr class="equ">
		...
		<td class="equ">{{ final_result["fiss"]["dce_score_mean"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">std</th>
		<td class="equ">{{ final_result["fiss"]["dce_score_std"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">median</th>
		<td class="equ">{{ final_result["fiss"]["dce_score_median"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">1st/3rd quartiles</th>
		<td class="equ">{{ final_result["fiss"]["dce_score_q1"] }}, {{ final_result["fiss"]["dce_score_q3"] }}</td>
	</tr>
	<tr class="equ">
	...
	<tr class="equ">
		<th class="equ">mean</th>
		<td class="equ">{{ final_result["fiss"]["dice_coefficient_mean"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">std</th>
		<td class="equ">{{ final_result["fiss"]["dice_coefficient_std"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">median</th>
		<td class="equ">{{ final_result["fiss"]["dice_coefficient_median"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">1st/3rd quartiles</th>
		<td class="equ">{{ final_result["fiss"]["dice_coefficient_q1"] }}, {{ final_result["fiss"]["dice_coefficient_q3"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">Number annotated cases:</th>
		<td class="equ">{{ final_result["fiss"]["num_annotated"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">Number good predictions:</th>
		<td class="equ">{{ final_result["fiss"]["num_good_markings"] }} cases with meeting {{ final_result["fiss"]["threshold"] }} threshold</td>
	</tr>
	<!-- <tr class="equ">
		<td class="equ">Bad markings:</td>
		<td class="equ">{{ final_result["fiss"]["num_bad_markings"] }}</td>
	</tr> -->
	<!-- <tr class="equ">
		...
		<td class="equ">
			<table class="list" id="t02">
				<tr class="list">
					<td class="list">TP: {{ final_result["fiss"]["tp"] }}</td>
					<td class="list">FP: {{ final_result["fiss"]["fp"] }}</td>
				</tr>
				<tr class="list">
					<td class="list">FN: {{ final_result["fiss"]["fn"] }}</td>
					<td class="list">TN: {{ final_result["fiss"]["tn"] }}</td>
				</tr>
			</table>
	...
	<tr class="equ">
		...
		<td class="equ">{{ final_result["fiss"]["sensitivity"] }}</td>
	</tr>
	<tr class="equ">
		<th class="equ">Specificity:</th>
		<td class="equ">{{ final_result["fiss"]["specificity"] }}</td>
	</tr>
...
{% if final_result["bad_cases"]["fiss"] -%}
...
{% for key in final_result["bad_cases"]["fiss"] -%}
	<tr class="list">
		<td class="list"><a href="sub_report/{{key}}.html">{{key}}</a></td>
		<td class="list">{{final_result["per_case_result"][key]["fiss"].get("dce_score")}}</td>
		<td class="list">{{final_result["per_case_result"][key]["fiss"].get("dice_coefficient")}}</td>
		<!-- <td class="list">{{final_result["per_case_result"][key]["fiss"].get("class")}}</td> -->
	</tr>
{%- endfor %}
</table>
{%- endif %}
...
{% if final_result["good_cases"]["fiss"] -%}
<h1>Other cases</h1>
<table class="list">
	<tr class="list">
		<th class="list">Case</th>
...
{% for key in final_result["good_cases"]["fiss"] -%}
	<tr class="list">
		<td class="list"><a href="sub_report/{{key}}.html">{{key}}</a></td>
		<td class="list">{{final_result["per_case_result"][key]["fiss"].get("dce_score")}}</td>
		<td class="list">{{final_result["per_case_result"][key]["fiss"].get("dice_coefficient")}}</td>
