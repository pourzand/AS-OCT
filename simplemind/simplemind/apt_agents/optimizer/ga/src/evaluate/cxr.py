...
from .plotting_cxr import generate_screenshot, generate_markings, gen_hist_plot, gen_scatter_plot
from simplemind.apt_agents.optimizer.ga.src.core.evaluate import Evaluator, ResultsCompiler, MIU_Result, get_xy, compute_point_error, point_detection
...
class MIU_Result_CXR(MIU_Result):
...
def read_miu_results(results_path, rois=None, visuals=None):
    result = MIU_Result_CXR(results_path)
...
# 'CVC': 0.2,
        ...
        # visuals = ["carina_cnn_*.png",]
        # roi_dict = {"carina_cnn": "Crina"}  # roi_miu: roi_ref
        # rois = list(roi_dict.keys())
        # self.point_rois = ["carina_cnn",]

        ## GE jct has a point_roi too


class Evaluator_CXR(Evaluator):
    def __init__(self):
        visuals = ["ng_cnn_*.png", "ettip_cnn_*.png", "carina_cnn_*.png", "gejct_cnn_*.png"]        ## if you want to include overlay visualizations, you can change this :)
        # roi_dict = {"ng_cnn": "NgTub", "ettip_cnn": "EtTip", "carina_cnn": "Crina", "gejct_cnn": "GEjct",}    ## in case you want to evaluate just CNN
        roi_dict = {"ng_tube": "NgTub", "et_tip": "EtTip", "carina": "Crina", "gejct": "GEjct",}
        rois = list(roi_dict.keys())
        self.point_rois = ["et_tip", "carina", "gejct"]
    ...
    def load_reference(self, reference_path, image=None, dataset=None, id=None):
        ...
        ### Enable if we want to use TraCh ROI ###
        ...
        ### Enable if we want to use NgTub roi ###
        # reference_contents["NgTub"]["roi"] = None
        # if reference_contents["NgTub"].get("roi_path") is not None and image is not None:
            # try:
            #     from simplemind import __qia__
            #     import qia.common.img.image as qimage
            # except:
            #     print("Failed to import qimage (evaluate.py)")
        #     roi = qimage.cast(image)
        #     roi.fill_with_roi(reference_contents["NgTub"]["roi_path"])
    ...
    def _compute_evaluation_measures_comprehensive(self, image, rois, reference, outpath, skip_images=False, extra=None):
        if not reference:
        ...
        for miu_key, ref_key in self.roi_dict.items():
            ...
            ### if it is a point roi, do special point roi evaluation
            if miu_key in self.point_rois:
            ...
            ### otherwise, do regular segmentation evalutaion
            else:
...
def evaluate_task(param, previous_output=None):
    ...
    evaluator  = Evaluator_CXR()    ## TODO: potentially generalizable if evaluator is passed through param
...
class ResultsCompiler_CXR(ResultsCompiler):
    def __init__(self, fitness_func=fitness_cxr, overall_report_template=DEFAULT_TEMPLATE, casewise_report_template=DEFAULT_SUB_TEMPLATE ):
        ...
        self.ref_keys = ["Crina", "GEjct", "EtTip", "NgTub",] # markings/outputs we're interested in    ### TODO: add TraCh/CVC when ready

        ### This section for population-wise visualization, which is optional ###
        self.hist_plot_dict = {'Crina': { "metric": "tot_err", 
                                "title": "Carina", 
                                "xlabel": "error [mm]",},
                    'GEjct': { "metric": "tot_err", 
                                "title": "GE Junction", 
                                "xlabel": "error [mm]",},
                    'EtTip': { "metric": "tot_err", 
                                "title": "ETT Tip", 
                                "xlabel": "error [mm]",},
                    'NgTub': { "metric": "dice_coefficient", 
                                "title": "NG Tube Overall", 
                                "xlabel": "DCE [%]",},

        }
        self.scatter_plot_dict = {'Crina': 
                                {"title": "Carina", 
                                "x_metric": "x_err", 
                                "y_metric": "y_err", 
    ...
    def stratify_performance(self, labelwise_result, ref_key, ):
        ...
        if labelwise_result.get("dce_score") is not None:
            if labelwise_result["dce_score"] < self.thresh[ref_key]:
                return False
            return True
        elif labelwise_result.get("tot_err") is not None:
    ...
    def overall_visualization(self, df, outpath):
        vis_dict = dict()

        sp = os.path.join(outpath, 'error_hist.png')
        vis_dict['error_hist_plot'] = gen_hist_plot(df, sp, self.hist_plot_dict)

        sp = os.path.join(outpath, 'error_scatter.png')
        vis_dict['error_scatter_plot'] = gen_scatter_plot(df, sp, self.scatter_plot_dict)

        return vis_dict
    def compile_function_comprehensive(self, result_dictionary, outpath,
                        rerun=False,
                        canary=False):
        ...
        cols = pd.MultiIndex.from_product([self.ref_keys,
                                        ['ref_detection', 'pred_detection', 'dce_score', 'dice_coefficient', 'x_err', 'y_err', 'tot_err']],
                                        names=['mtype', 'value'])
        ...
        final_result.update(self.overall_visualization(df,outpath))
...
def compile_function(param, previous_output=None):
    ...
    results_compiler  = ResultsCompiler_CXR(overall_report_template=report_templates.get("report"), casewise_report_template=report_templates.get("subreport"))
