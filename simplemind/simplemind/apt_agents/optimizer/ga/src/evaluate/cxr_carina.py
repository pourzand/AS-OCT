...
class Evaluator_CXR_Carina(Evaluator):
    def __init__(self):
...
def evaluate_task(param, previous_output=None):
    ...
    evaluator  = Evaluator_CXR_Carina()    ## TODO: potentially generalizable if evaluator is passed through param
...
def fitness_cxr(final_result):
    ...
    if isnan(final_result["Crina"]["tot_err_raw"]):
        return -1000000
    elif final_result["Crina"]["tot_err_raw"] is None:
        return -2000000
    else:
        return -1 * (final_result["Crina"]["tot_err_raw"] + 10*(final_result["Crina"]["fn"]))
    
from simplemind.apt_agents.optimizer.ga.src.conf import CONFIG_PATH
DEFAULT_TEMPLATE = os.path.join(CONFIG_PATH, "cxr", "carina_template.tpl")
DEFAULT_SUB_TEMPLATE = os.path.join(CONFIG_PATH, "cxr", "carina_sub_template.tpl")

###################

class ResultsCompiler_CXR_Carina(ResultsCompiler):
    def __init__(self, fitness_func=fitness_cxr, overall_report_template=DEFAULT_TEMPLATE, casewise_report_template=DEFAULT_SUB_TEMPLATE ):
        ...
        self.ref_keys = ["Crina",] # markings/outputs we're interested in
...
def compile_function(param, previous_output=None):
    ...
    results_compiler  = ResultsCompiler_CXR_Carina(overall_report_template=report_templates.get("report"), casewise_report_template=report_templates.get("subreport"))
