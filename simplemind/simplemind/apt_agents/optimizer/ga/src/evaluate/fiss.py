...
"""
class MIU_Result_Fissure(MIU_Result):
    def load_image(self, ):
    ...
    def load_rois(self, ):
        ...
        if rois.get('fiss_cnn') is not None:
            seg = np.array(rois.get('fiss_cnn').get_array())

        elif rois.get('fiss_cnn') is None:
            image = np.array(self.image.get_array())
...
def read_miu_results(results_path, rois=None, visuals=None):
    ...
    result = MIU_Result_Fissure(results_path)
...
class Evaluator_Fissure(Evaluator):
    def __init__(self):
    ...
    def load_reference(self, reference_path, image=None, dataset=None, id=None):
        ...
        reference_contents = {}
        reference_contents['fiss'] = {'roi_path':roi_path, 'roi':ref_roi_np}
    ...
    def casewise_visualization(self, outpath, image, case_dict, rois, extra):
        ...
        vis_dict["marking_ss"] = view_img(image, rois['fiss'], marking_path)    #generate_markings(image_arr, vis_dict, rois, marking_path)
        vis_dict["original_ss"] = view_img(image, case_dict['fiss']['reference_roi'], image_path)#generate_screenshot(image_arr, vis_dict, rois, image_path)
...
def evaluate_task(param, previous_output=None):
    ...
    evaluator  = Evaluator_Fissure()    ## TODO: potentially generalizable if evaluator is passed through param
...
def fitness(final_result):
    return final_result["fiss"]["dce_score_raw"]
    
from simplemind.apt_agents.optimizer.ga.src.conf import CONFIG_PATH
DEFAULT_TEMPLATE = os.path.join(CONFIG_PATH, "fissure", "fissure_template.tpl")
DEFAULT_SUB_TEMPLATE = os.path.join(CONFIG_PATH, "fissure", "fissure_sub_template.tpl")

###################

class ResultsCompiler_Fissure(ResultsCompiler):
    def __init__(self, fitness_func=fitness, overall_report_template=DEFAULT_TEMPLATE, casewise_report_template=DEFAULT_SUB_TEMPLATE ):
        ...
        self.ref_keys = ["fiss",] # markings/outputs we're interested in
    ...
    def compile_function_fissure(self, result_dictionary, outpath,
                        rerun=False,
                        canary=False):
...
def compile_function(param, previous_output=None):
    ...
    results_compiler  = ResultsCompiler_Fissure(overall_report_template=report_templates.get("report"), casewise_report_template=report_templates.get("subreport"))
