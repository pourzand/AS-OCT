...
def view_img(qia_image,roi_arr, path, color):
    ...
    try:
        c = 1
        plt.figure(0, figsize=(c,c*len(image_arr)))
...
class MIU_Result_Renal(MIU_Result):
    ...
    def load_rois(self, ):
        ...
        if rois.get('kidneys'):
            seg = np.array(rois.get('kidneys').get_array()) 
        elif rois.get('kidney_final_right') is not None and rois.get('kidney_final_left') is not None:
...
class Evaluator_Renal(Evaluator):
    def __init__(self):
        ...
        visuals = None
        roi_dict = {'kidneys': 'combined_kidneys'}
    ...
    def casewise_visualization(self, outpath, image, case_dict, rois, extra):
        ...
        vis_dict["marking_ss"] = view_img(image, rois['kidneys'], marking_path, 'autumn')    #generate_markings(image_arr, vis_dict, rois, marking_path)
        ...
        if extra.get("visuals") is not None:
            if extra["visuals"]:
                vis_dict["preprocessed_ss"] = extra["visuals"][0]
        return vis_dict
...
DEFAULT_TEMPLATE = os.path.join(CONFIG_PATH, "renal", "kidney_template.tpl")
DEFAULT_SUB_TEMPLATE = os.path.join(CONFIG_PATH, "renal", "kidney_sub_template.tpl")
...
def compile_function(param, previous_output=None):
    ...
    results_compiler  = ResultsCompiler_Renal(overall_report_template=report_templates.get("report", DEFAULT_TEMPLATE), casewise_report_template=report_templates.get("subreport", DEFAULT_SUB_TEMPLATE))
