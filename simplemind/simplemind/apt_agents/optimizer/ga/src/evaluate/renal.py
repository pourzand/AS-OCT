...
class Evaluator_Renal(Evaluator):
    ...
    def casewise_visualization(self, outpath, image, case_dict, rois, extra):
        ...
        vis_dict["marking_ss"] = view_img(image, rois['combined_kidneys'], marking_path)    #generate_markings(image_arr, vis_dict, rois, marking_path)
        vis_dict["original_ss"] = view_img(image, case_dict['combined_kidneys']['reference_roi'], image_path)#generate_screenshot(image_arr, vis_dict, rois, image_path)
    ...
    def _custom_dce_score(self, reference, rois, ref_key, miu_key):
        ...
        tn_list = []
        fn_list = []
        vector_gt = gt.flatten()
        vector_seg = seg.flatten()
        
        for i in range(len(vector_gt)):

          if vector_gt[i] == 0 and vector_seg[i] == 0:
              tn_list.append(1)

          elif vector_gt[i] == 1 and vector_seg[i] == 0:
              fn_list.append(1)
        ...
        acc = (tp + tn)/(tp + tn + 1.5*fp + fn) # TN is too large so acc function tends to one, basically limit to infinity

        mod_dice = 2*tp / ((tp + 3*fp) + (tp + fn))

        return dict(dce_score=mod_dice, dice_coefficient=dice, accuracy=acc, )
    ...
    def _compute_evaluation_measures_segmentation(self, image, rois, reference, outpath, skip_images=False, extra=None):
        ...
        for miu_key, ref_key in self.roi_dict.items():
            ...
            case_dict[ref_key] = dict(  dce_score=float(dce_dict["dce_score"]), 
                                        ...
                                        accuracy=float(dce_dict["accuracy"]),
                                        marking=None,
...
class ResultsCompiler_Renal(ResultsCompiler):
    ...
    def compile_function_renal(self, result_dictionary, outpath,
        ...
        cols = pd.MultiIndex.from_product([self.ref_keys,
                                        ['dce_score', 'dice_coefficient','accuracy']],
                                        names=['mtype', 'value'])
            ...
            for k, case_dict in result_dictionary.items():
                for mtype, labels in case_dict.items():
                    ...
                    if mtype in self.ref_keys:
                        ...
                        df.loc[k, (mtype, 'accuracy')] = labels.get('accuracy')
        ...
        for mtype in self.ref_keys:
            ...
            final_result[mtype].update(self._add_performance_metrics(marking_df, "accuracy"))
