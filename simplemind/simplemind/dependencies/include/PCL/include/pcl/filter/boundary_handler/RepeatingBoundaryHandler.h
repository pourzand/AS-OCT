#ifndef PCL_REPEATING_BOUNDARY_HANDLER
#define PCL_REPEATING_BOUNDARY_HANDLER
...
namespace pcl
{
	namespace filter
	{
		...
		class RepeatingBoundaryHandler: private boost::noncopyable
		{
		public:
			RepeatingBoundaryHandler() {}
			RepeatingBoundaryHandler(const typename ImageType::ConstantPointer& img) 
			{
				setImage(img);
			}
			
			RepeatingBoundaryHandler& operator=(const RepeatingBoundaryHandler& obj)
			{
				return *this;
			}
