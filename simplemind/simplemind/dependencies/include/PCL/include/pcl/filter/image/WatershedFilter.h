#ifndef PCL_WATERSHED_FILTER
#define PCL_WATERSHED_FILTER
...
namespace pcl
{
	namespace filter
	{
		...
		class WatershedFilter: public ImageFilterBase
		{
		public:
			static typename OutputImageType::Pointer Compute(const typename InputImageType::ConstantPointer& input, const pcl::iterator::ImageNeighborIterator::ConstantOffsetListPointer& neighborhood, const Region3D<int>& process_region=Region3D<int>().reset())
			{
				WatershedFilter filter;
			...
			InternalImageType::Pointer computeFlowImage()
			{
				...
				if (!region_gen.getSafeRegion().empty()) {
					...
					pcl_ForIterator(iter) {
						...
						pcl_ForIterator(m_NeighborIter) {
				...
				pcl_ForEach(region_gen.getUnsafeRegion(), item) {
					...
					pcl_ForIterator(iter) {
						...
						pcl_ForIterator(m_NeighborIter) if (this->m_ProcessRegion.contain(m_NeighborIter.getPoint())) {
							double gradient = (m_Input->get(iter)-m_Input->get(m_NeighborIter))/m_Delta[m_NeighborIter.getIteration()];
							if (gradient>0 && gradient>steepest_flow) {
								steepest_flow = gradient;
								steepest_direction = m_NeighborIter.getIteration();
