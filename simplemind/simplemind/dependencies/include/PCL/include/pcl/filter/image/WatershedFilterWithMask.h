#ifndef PCL_WATERSHED_FILTER_WITH_MASK
#define PCL_WATERSHED_FILTER_WITH_MASK
...
namespace pcl
{
	namespace filter
	{
		...
		template <class InputImageType, class MaskImageType, class OutputImageType>
		class WatershedFilterWithMask: public ImageFilterBase
		{
		public:
			static typename OutputImageType::Pointer Compute(const typename InputImageType::ConstantPointer& input, const typename MaskImageType::ConstantPointer& mask, const pcl::iterator::ImageNeighborIterator::ConstantOffsetListPointer& neighborhood, const Region3D<int>& process_region=Region3D<int>().reset())
			{
				WatershedFilterWithMask filter;
			...
			void setMask(const typename MaskImageType::ConstantPointer& mask)
			{
				m_Mask = mask;
			}
			...
			void update()
			{
				...
				pcl_ForIterator(image_iter) if (m_Mask->get(image_iter) && flow_image->get(image_iter)==-1) {
					sink_count++;
					m_Output->set(image_iter, sink_count);
					BruteRegionGrowingIterator grow_iter;
					grow_iter.setNeighborIterator(m_NeighborIter, this->m_ProcessRegion);
					grow_iter.addSeed(image_iter.getPoint(), image_iter);
					pcl_ForIterator(grow_iter) if (m_Mask->get(grow_iter) && m_Output->get(grow_iter)==0) {
		...
		protected:
			...
			typename MaskImageType::ConstantPointer m_Mask;
			...
			InternalImageType::Pointer computeFlowImage()
			{
				...
				misc::SafeUnsafeRegionGenerator region_gen(this->m_ProcessRegion, this->m_ProcessRegion, m_NeighborIter.getOffsetRegion());
				...
				if (!region_gen.getSafeRegion().empty()) {
					...
					pcl_ForIterator(iter) if (m_Mask->get(iter)) {
						double steepest_flow = 0;
						int steepest_direction = -1;
						m_NeighborIter.setOrigin(iter);
						pcl_ForIterator(m_NeighborIter) if (m_Mask->get(m_NeighborIter)) {
				...
				pcl_ForEach(region_gen.getUnsafeRegion(), item) {
					...
					pcl_ForIterator(iter) if (m_Mask->get(iter)) {
						double steepest_flow = 0;
						int steepest_direction = -1;
						m_NeighborIter.setOrigin(iter.getPoint(), iter);
						pcl_ForIterator(m_NeighborIter) if (this->m_ProcessRegion.contain(m_NeighborIter.getPoint()) && m_Mask->get(m_NeighborIter)) {
							double gradient = (m_Input->get(iter)-m_Input->get(m_NeighborIter))/m_Delta[m_NeighborIter.getIteration()];
							if (gradient>0 && gradient>steepest_flow) {
								steepest_flow = gradient;
								steepest_direction = m_NeighborIter.getIteration();
			...
			void correctFlowImage(InternalImageType::Pointer& flow_image)
			{
				...
				pcl_ForIterator(image_iter) if (m_Mask->get(image_iter) && flow_image->get(image_iter)==-1) {
					...
					if (decending_flow!=-1) {
						...
						reggrow_ptr->setNeighborIterator(m_NeighborIter, this->m_ProcessRegion);
				...
				while (!done) {
					...
					pcl_ForEach(region_growing_list, item) if (*item) {
						...
						for(; !iter.end(); iter.next()) if (m_Mask->get(iter) && flow_image->get(iter)==-1) {
				...
				pcl_ForIterator(image_iter) if (m_Mask->get(image_iter) && flow_image->get(image_iter)==-1) {
					...
					iter.setNeighborIterator(m_NeighborIter, this->m_ProcessRegion);
					iter.addSeed(image_iter.getPoint(), image_iter);
					pcl_ForIterator(iter) if (m_Mask->get(iter) && iter.getIndex()!=image_iter.getIndex() && flow_image->get(iter)==-1) {
						Point3D<int> offset(iter.getSourcePoint());
						offset -= iter.getPoint();
						flow_image->set(iter, toOffsetIndex(offset));
						iter.accept();
