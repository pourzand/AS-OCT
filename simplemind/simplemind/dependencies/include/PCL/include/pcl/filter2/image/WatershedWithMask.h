#ifndef PCL2_WATERSHED_WITH_MASK
#define PCL2_WATERSHED_WITH_MASK
...
namespace pcl
{
	namespace filter
	{
		...
		class WatershedWithMask
		{
		public:
			static typename OutputImageType::Pointer Compute(const typename InputImageType::ConstantPointer& input, const typename MaskImageType::ConstantPointer& mask, const pcl::iterator::ImageNeighborIterator::ConstantOffsetListPointer& neighborhood)
			{
				WatershedWithMask filter;
		...
		protected:
			...
			int findDecendingNeighbor(InternalImageType::Pointer& flow_image, typename InputImageType::ConstantValueType origin_val) 
			{
				...
				pcl_ForIterator(m_NeighborIter) if (m_Input->contain(m_NeighborIter.getPoint()) && m_Mask->get(m_NeighborIter)) {
