...
namespace pcl
{
	namespace gui
	{
		...
		protected:
			...
			vtk_Pointer(vtkLineSource, m_LineSource[2]);
			vtk_Pointer(vtkActor, m_LineActor[2]);
			...
			ImageCrossHairs(vtkMatrix4x4* axes, double *normal_multiplier, double length_ratio, double offset)
			{
				...
				m_Offset = offset;
				for (int i=0; i<2; ++i) {
					m_LineSource[i] = vtkLineSource::New();
					vtk_NewPointer(vtkPolyDataMapper, mapper);
					mapper->SetInput(m_LineSource[i]->GetOutput());
					m_LineActor[i] = vtkActor::New();
