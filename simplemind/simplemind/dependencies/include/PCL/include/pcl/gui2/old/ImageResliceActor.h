...
namespace pcl
{
	namespace gui
	{
		...
		protected:
			...
			int m_Extent[4];
			double m_Spacing[2];
			vtk_Pointer(vtkMatrix4x4, m_Axes);
		...
		class ImageResliceActor: public ModifiableObject
		{
		public:
			...
			{
				if (en) m_ImageActor->VisibilityOn();
				else m_ImageActor->VisibilityOff();
			}
			
			void updateColorLookup()
			{
				vtk_NewPointer(vtkImageMapToColors, color);
				color->SetLookupTable(m_ImageSource->getColorLookup());
		...
		protected:
			...
			vtk_Pointer(vtkImageReslice, m_ImageReslice);
			vtk_Pointer(vtkImageActor, m_ImageActor);
	
			ImageResliceActor(const ImageSourceBase::Pointer& image_source)
			{
				m_Param = ImageResliceParam::New();
				vtk_NewPointer(vtkMatrix4x4, axes);
