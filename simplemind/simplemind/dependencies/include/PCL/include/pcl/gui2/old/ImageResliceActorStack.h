...
#include <pcl/gui2/ImageResliceActor.h>
#include <pcl/gui2/ImageCrossHairs.h>
...
namespace pcl
{
	namespace gui
	{
		...
		protected:
			...
			vtk_Pointer(vtkMatrix4x4, m_ResliceAxes);
			vtk_Pointer(vtkImageStack, m_ImageStack);
			...
			{
				m_ImageStack->GetImages()->RemoveAllItems();
				if (m_Image.empty() && m_Segment.empty()) return;
