#ifndef PCL_VORONOI_MAP_GENERATOR_WITH_MASK
#define PCL_VORONOI_MAP_GENERATOR_WITH_MASK
...
namespace pcl
{
	namespace misc
	{
		...
		/**
			Note that different from the without mask version, 0 is reserved for regions outside the mask!
		**/
		template <class MaskImageType, class OutputImageType>
		class VoronoiMapGeneratorWithMask
		{
		public:
			typedef typename OutputImageType::IoValueType OutputValueType;

			VoronoiMapGeneratorWithMask() 
			{
				setUseSpacing(true);
			}
			template <class ImagePointer>
			VoronoiMapGeneratorWithMask(const typename MaskImageType::ConstantPointer& mask, const ImagePointer& template_image)
			{
			...
			void setOutputBasedOn(const ImagePointer& template_image)
			{
				...
				m_Seed.clear();
				m_Seed.push_back(Point3D<int>(
					boost::numeric::bounds<OutputValueType>::lowest(),
					boost::numeric::bounds<OutputValueType>::lowest(),
					boost::numeric::bounds<OutputValueType>::lowest()
					));
			}
			...
			{
				if (m_Output->get(seed)!=0 || !(m_Mask->get(seed))) return false;
			...
			{
				if (m_Output->get(index)!=0 || !(m_Mask->get(index))) return false;
				m_Seed.push_back(seed);
