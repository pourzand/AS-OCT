#ifndef PCL_LAPLACIAN_SMOOTHING_FILTER_WITH_BETA_MAP
#define PCL_LAPLACIAN_SMOOTHING_FILTER_WITH_BETA_MAP
...
namespace pcl
{
	namespace terrain_map_filter
	{
		...
		template <class Type, class BetaMapType>
		class LaplacianSmoothingFilterWithBetaMap: public boost::noncopyable
		{
		public:
			typedef LaplacianSmoothingFilterWithBetaMap Self;
			typedef boost::shared_ptr<Self> Pointer;
			typedef Type ResultType;

			template <class InputPointertType>
			static Pointer New(typename InputPointertType& input, const typename BetaMapType::ConstantPointer& beta)
			{
			...
			void setBeta(const typename BetaMapType::ConstantPointer& beta)
			{
				m_Beta = beta;
			}
			...
			void update()
			{
				...
				if (m_SafeRegionExist) {
					pcl_ForIterator(m_SafeIterator) {
						...
						laplacian_point /= 4.;
						m_Result->set(m_SafeIterator, cur_val+m_Beta->get(m_SafeIterator)*(laplacian_point-cur_val));
					}
				}
				pcl_ForEach(m_UnsafeRegionList, item) {
					...
					pcl_ForIterator(m_UnsafeIterator) {
						...
						laplacian_point /= num;
						m_Result->set(m_UnsafeIterator, cur_val+m_Beta->get(m_UnsafeIterator)*(laplacian_point-cur_val));
					}
				}
			}
		...
		protected:
			...
			typename BetaMapType::ConstantPointer m_Beta;
			...
			LaplacianSmoothingFilterWithBetaMap() {}
