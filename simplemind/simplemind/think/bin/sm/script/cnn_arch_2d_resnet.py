"""Two-dimensional CNN architectures: ResNet v1, v2, and segmentation models for Keras.

Method
------
ResNet
    Builds a custom ResNet like architecture. Defaults to ResNet50 v2.
ResNet18(input_shape, classes, dropout=None):
    ResNet with 18 layers and v2 residual units
    ResNet(input_shape, classes, basic_block, activation='linear',
        dropout=dropout, repetitions=[2, 2, 2, 2])
ResNet34(input_shape, classes, dropout=None):
    ResNet with 34 layers and v2 residual units
    ResNet(input_shape, classes, basic_block, activation='linear',
        dropout=dropout,repetitions=[3, 4, 6, 3])
ResNet50(input_shape, classes)
    ResNet with 50 layers and v2 residual units
    ResNet(input_shape, classes, bottleneck, repetitions=[3, 4, 6, 3])
ResNet101(input_shape, classes)
    ResNet with 101 layers and v2 residual units
    ResNet(input_shape, classes, bottleneck, repetitions=[3, 4, 23, 3])
ResNet152(input_shape, classes)
    ResNet with 152 layers and v2 residual units
    ResNet(input_shape, classes, bottleneck, repetitions=[3, 8, 36, 3])
...
def ResNet(input_shape=None, classes=10, block='bottleneck', residual_unit='v2',
    ...
    Returns:
        The keras `Model`.
    """
    if activation not in ['softmax', 'sigmoid', 'linear', None]:
